...
const char * const BITCOIN_CONF_FILENAME = "plutoncoin.conf";
...
#else
    const char* pszModule = "plutoncoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PlutonCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PlutonCoin";
#else
    // Unix
    return pathRet / ".plutoncoin";
#endif
#endif
}
